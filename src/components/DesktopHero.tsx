...
//  Full 240-frame scroll-driven canvas animation.
//  This file is intentionally imported with next/dynamic({ ssr: false })
//  to prevent framer-motion useScroll/useTransform from causing hydration
//  mismatches between server-rendered HTML and client DOM.
// ─────────────────────────────────────────────────────────────────
...
export default function DesktopHero() {
  ...
  return (
    <div ref={containerRef} className="h-[500vh] bg-background relative w-full">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">

        {/* Parallax radial violet lighting orb */}
        <motion.div style={{ y: orbY }} className="absolute inset-0 z-0 pointer-events-none">
          <div
        ...
        <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
          {particles.map((p) => (
            <motion.div
              ...
              animate={{ y: [0, -30, 0], opacity: [p.opacity * 0.5, p.opacity, p.opacity * 0.5], scale: [1, 1.2, 1] }}
        ...
        >
          <motion.div style={{ opacity: opacity1 }} className="absolute">
            <h1
              className="text-8xl font-bold text-white tracking-widest mb-4"
              style={{ textShadow: "0 0 60px rgba(157,77,255,0.5), 0 0 120px rgba(106,0,255,0.3)" }}
            >
              DUALDEER
            </h1>
            <p className="text-3xl text-gradient">Engineered for speed.</p>
          </motion.div>

          <motion.div style={{ opacity: opacity2 }} className="absolute">
            <h2
              className="text-7xl text-white mb-4 font-bold"
              style={{ textShadow: "0 0 40px rgba(157,77,255,0.4)" }}
            >
              Precision Compression.
            </h2>
            <p className="text-2xl" style={{ color: "#C084FF" }}>Engineered muscle support.</p>
          </motion.div>

          <motion.div style={{ opacity: opacity3 }} className="absolute">
            <h2
              className="text-7xl text-white max-w-4xl font-bold"
              style={{ textShadow: "0 0 40px rgba(157,77,255,0.4)" }}
            >
          ...
          <motion.div style={{ opacity: opacity4 }} className="absolute mt-32">
            ...
            <Link
              href="/shop"
              className="pointer-events-auto px-8 py-4 font-semibold rounded-full transition-all shadow-lg text-white inline-block"
              style={{
        ...
        {/* Loading screen — fades out once frames are ready */}
        <AnimatePresence>
          {!loaded && (
            <motion.div
              initial={{ opacity: 1 }}
            ...
            >
              ...
              <div className="w-48 rounded-full overflow-hidden" style={{ height: "1px", background: "#1a1a2e" }}>
                <div
                  ...
                  style={{ width: `${loadProgress}%`, background: "linear-gradient(to right, #6A00FF, #C084FF)" }}
                />
              </div>
