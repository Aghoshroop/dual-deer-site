...
export default function PerformanceSection() {
  ...
  useEffect(() => {
    const loadImage = async () => {
      ...
      const stored = settings.performanceImage;
      if (!stored) return;
      // Could be a data URL or an imageStore ID
      if (stored.startsWith("data:") || stored.startsWith("http") || stored.startsWith("/")) {
        setImageSrc(stored);
      } else {
        // It's an IndexedDB imageStore key
        const url = await getImage(stored);
          ...
          <div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start">
                <div className="h-px w-12 bg-accent-blue mt-3 mr-4"></div>
                <div>
                  <h4 className="text-white font-semibold text-lg">64-Layer Carbon Micro-Fiber</h4>
                  <p className="text-sm text-gray-500 mt-1">Military-grade compression substrate — 40% stronger than standard compression fabric.</p>
                </div>
              </div>
            </div>
          </div>

          {/* Right panel — shows image if admin has uploaded one, else animated placeholder */}
          <div className="relative h-[600px] w-full rounded-2xl overflow-hidden glass-panel group">
            <div className="absolute inset-0 bg-gradient-to-tr from-accent-blue/10 to-transparent mix-blend-overlay group-hover:from-accent-blue/20 transition-all duration-700" />

            {imageSrc ? (
              /* Admin-uploaded performance image */
              <>
              ...
              /* Animated placeholder — shows until admin uploads an image */
              <>
                ...
                {/* Upload hint for site owners (only visible in dev / when no image set) */}
